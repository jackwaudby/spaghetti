...
sgt = fromJSON(file = paste0("../results/",params$workload,"/sgt-sf1.json"))
hit = fromJSON(file = paste0("../results/",params$workload,"/hit-sf1.json"))
tpl = fromJSON(file = paste0("../results/",params$workload,"/2pl-sf1.json"))
opt_hit = fromJSON(file = paste0("../results/",params$workload,"/opt-hit-sf1.json"))
...
kable(dat, caption = paste0(params$workload," workload."),col.names = c('Protocol', 'Abort Rate (%)','Throughput (Txn/s)'))


kable(latency,
      caption = paste0("Latency (ms) for ",params$workload," workload."))
```
...
```{r sgt, echo=FALSE}
kable(sgt_aborts, caption = paste0("SGT abort breakdown for ",params$workload," workload."))
```
...
```{r tpl, echo=FALSE}
kable(tpl_aborts, caption = paste0("2PL abort breakdown for ",params$workload," workload."))
```
...
```{r hit, echo=FALSE}
kable(hit_aborts,caption = paste0("Wait-hit abort breakdown for ",params
                                  $workload," workload."))
```
...
kable(opt_hit_aborts,caption = paste0("Optimised wait-hit abort breakdown for ",params$workload," workload."))
```
