...
# load data
col_names = c("sf","protocol","workload","cores","total_time","commits","aborts","errors","total_latency")
...
if (include_2pl == T) {
  durner_2pl = read_delim(file = './data/durner_2pl.csv', delim = ";",col_names = F)
...
if (include_sgt == T){
  durner <- read_delim(file = './data/durner_sgt.csv', delim = ";",col_names = F)
...
if (nrow(sf1) > 0) {
  ggplot(data=sf1, aes(x=cores, y=thpt, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("thpt (million/s)") +
    ggtitle(paste0(workload," High Contention")) +
    theme_bw() 
  
  ggsave(paste0("./graphics/",workload,"_thpt_sf1.png"))
  
  ggplot(data=sf1, aes(x=cores, y=abr, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("abort rate") +
    ggtitle(paste0(workload," High Contention")) +
    theme_bw()

  ggsave(paste0("./graphics/",workload,"_abort_rate_sf1.png"))
  
  
  ggplot(data=sf1, aes(x=cores, y=lat, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("av latency (ms)") +
    ggtitle(paste0(workload," High Contention")) +
    theme_bw()
  
  ggsave(paste0("./graphics/",workload,"_lat_sf1.png"))
}

if (nrow(sf3) > 0) {
  
  ggplot(data=sf3, aes(x=cores, y=thpt, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("thpt (million/s)") +
    ggtitle(paste0(workload," Low Contention")) +
    theme_bw() 

  ggsave(paste0("./graphics/",workload,"_thpt_sf3.png"))
  
  ggplot(data=sf3, aes(x=cores, y=abr, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("abort rate") +
    ggtitle(paste0(workload," Low Contention")) +
    theme_bw()
  ...
  ggsave(paste0("./graphics/",workload,"_abort_rate_sf3.png"))
  
  ggplot(data=sf3, aes(x=cores, y=lat, group=protocol, colour=protocol)) +
    geom_line() +
    ylab("av latency (ms)") +
    ggtitle(paste0(workload," Low Contention")) +
    theme_bw()

  ggsave(paste0("./graphics/",workload,"_lat_sf3.png"))
}
