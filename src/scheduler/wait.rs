...
use crate::scheduler::wait::transaction::PredecessorSet;
use crate::scheduler::wait::transaction::{Transaction, TransactionState};
...
impl<'a> Wait<'a> {
    ...
    pub fn begin(&self) -> TransactionId {
        ...
        *self.transaction_cnt.get_or(|| RefCell::new(0)).borrow_mut() += 1;
        ...
        Wait::EG.with(|x| x.borrow_mut().replace(guard));
    ...
    pub fn commit<'g>(&self, database: &Database) -> Result<(), NonFatalError> {
        ...
        loop {
            ...
            if delta >= watermark {
                self.abort(database);
                return Err(OptimisedWaitHitError::WaitedTooLong.into());
            }
...
impl PredecessorSummary {
    fn has_aborted_predecessor(&self) -> bool {
        self.aborted > 0
    }

    fn get_committed(&self) -> u64 {
        self.committed
    }
}

fn scan_predecessors<'a>(predecessors: &PredecessorSet<'a>) -> PredecessorSummary {
    let mut aborted = 0;
    let mut active = 0;
    let mut committed = 0;
