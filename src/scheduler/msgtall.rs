...
#[derive(Debug)]
pub struct AllMixedSerializationGraph {
...
impl AllMixedSerializationGraph {
    thread_local! {
    ...
    pub fn get_transaction(&self) -> *mut Node {
        AllMixedSerializationGraph::NODE.with(|x| *x.borrow().as_ref().unwrap())
    }
    ...
    pub fn cycle_check(&self) -> bool {
        ...
        let isolation = this.get_isolation_level();
    ...
    pub fn begin(&self, isolation_level: IsolationLevel) -> TransactionId {
        ...
        *self.txn_info.get_or(|| RefCell::new(None)).borrow_mut() =
        ...
        AllMixedSerializationGraph::EG.with(|x| x.borrow_mut().replace(guard)); // add to guard
    ...
    pub fn create_node(&self, isolation_level: IsolationLevel) -> (usize, usize, usize) {
        ...
        AllMixedSerializationGraph::NODE.with(|x| x.borrow_mut().replace(ptr)); // store in thread local
        ...
        loop {
            ...
            match this.msgt_has_incoming() {
                ...
                Incoming::SomeNotRelevant => {
                    ...
                    AllMixedSerializationGraph::EARLY.with(|x| x.borrow_mut().push((ops, node)));
    ...
    pub fn cleanup(&self, database: &Database) {
        ...
        AllMixedSerializationGraph::EG.with(|x| unsafe {
    ...
    fn delete_early_committers(&self, database: &Database) -> Vec<*mut Node> {
        ...
        AllMixedSerializationGraph::EARLY.with(|x| {
